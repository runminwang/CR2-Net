...
def prepare_for_boundary_segmentation(predictions, dataset):
    ...
    for image_id, prediction in tqdm(enumerate(predictions)):
        ...
        elif 'ctst' in cfg.DATASETS.TEST[0]:
            masks = [boundary_to_mask_ic(mask_x, mask_y, dataset.coco.imgs[original_id]["file_name"], number) for
                     mask_x, mask_y, number in zip(masks_x, masks_y,list(range(masks_x.shape[0])))]
        elif 'tpd' in cfg.DATASETS.TEST[0]:
            masks = [boundary_to_mask_ic(mask_x, mask_y, dataset.coco.imgs[original_id]["file_name"], number) for
                     mask_x, mask_y, number in zip(masks_x, masks_y,list(range(masks_x.shape[0])))]
        elif 'odett' in cfg.DATASETS.TEST[0]:
            masks = [boundary_to_mask_ic(mask_x, mask_y, dataset.coco.imgs[original_id]["file_name"], number) for
                     mask_x, mask_y, number in zip(masks_x, masks_y,list(range(masks_x.shape[0])))]
        elif 'mlt' in cfg.DATASETS.TEST[0]:
            masks = [boundary_to_mask_ic(mask_x, mask_y, dataset.coco.imgs[original_id]["file_name"], number) for
                     mask_x, mask_y, number in zip(masks_x, masks_y,list(range(masks_x.shape[0])))]
        elif 'ic13' in cfg.DATASETS.TEST[0]:
            masks = [boundary_to_mask_ic(mask_x, mask_y, dataset.coco.imgs[original_id]["file_name"], number) for
                     mask_x, mask_y, number in zip(masks_x, masks_y,list(range(masks_x.shape[0])))]
        else:
        ...
        scores = prediction.get_field("scores").tolist()
        labels = prediction.get_field("labels").tolist()
        # if 'ic15' or 'ctst' or 'tpd' or 'odett' in cfg.DATASETS.TEST[0]:
        #     rects = [mask_to_roRect(mask, [image_height, image_width]) for mask in masks]
        if 'ic15' or 'ctst' or 'tpd' or 'odett' or 'mlt' or 'ic13'in cfg.DATASETS.TEST[0]:
            rects = [mask_to_roRect(mask, [image_height, image_width]) for mask in masks]
        if 'CTW' in cfg.DATASETS.TEST[0]:
            contours = [mask_to_contours(mask, [image_height, image_width]) for mask in masks]
            # output for evaluation
            write_result_as_txt(image_name, contours, '/opt/data/private/ContourNet/output/ctw/inference/CTW1500_test/evaluation')
            # visualization
            if cfg.DATASETS.Test_Visual:
                im_write = cv2.imread('./datasets/CTW1500/test_img/'+ im_w_name)[:, :,::-1]
                for box in contours:
                    box = np.array(box)
                ...
                cv2.imwrite('./visualize/ctw/' + im_w_name,im_write[:, :, ::-1])
        # if 'odett' in cfg.DATASETS.TEST[0]:
        #     contours = [mask_to_contours(mask, [image_height, image_width]) for mask in masks]
        #     # output for evaluation
        #     write_result_as_txt(image_name, contours, '/opt/data/private/ContourNet/output/odett_new/inference/odett_test/evaluation')
        #     # visualization
        #     if cfg.DATASETS.Test_Visual:
        #         im_write = cv2.imread('./datasets/ODETT300/test/img/'+ im_w_name)[:, :,::-1]
        #         for box in contours:
        #             box = np.array(box)
        #             box = np.around(box).astype(np.int32)
        #             cv2.polylines(im_write[:, :, ::-1], [box.astype(np.int32).reshape((-1, 1, 2))], True, color=(0, 255, 0), thickness=2)  # 0,255,255 y 0,255,0 g
        #         cv2.imwrite('./visualize/odett/' + im_w_name,im_write[:, :, ::-1])

        if 'ic15' or 'ctst' or 'tpd' or 'odett' or 'ic13' in cfg.DATASETS.TEST[0]:
        # if 'ic15' or 'ctst' or 'tpd'  in cfg.DATASETS.TEST[0]:
            mapped_labels = [dataset.contiguous_category_id_to_json_id[i] for i in labels]
            esd = []
            ...
            if cfg.PROCESS.PNMS:
                ...
                if cfg.DATASETS.Test_Visual:
                    if 'ic15' in cfg.DATASETS.TEST[0]:
                        im_write = cv2.imread('./datasets/ic15/ic15/ic15_test_images/'+ im_w_name)[:, :,::-1]
                    
                    elif 'ctst' in cfg.DATASETS.TEST[0]:
                        im_write = cv2.imread('./datasets/CTST-1600/test_img/' + im_w_name)[:, :, ::-1]
                    elif 'tpd' in cfg.DATASETS.TEST[0]:
                        im_write = cv2.imread('./datasets/TPD/test_img/'+ im_w_name)[:, :,::-1]
                    elif 'odett' in cfg.DATASETS.TEST[0]:
                        im_write = cv2.imread('./datasets/ODETT300/test/img/'+ im_w_name)[:, :,::-1]
                    elif 'ic13' in cfg.DATASETS.TEST[0]:
                        im_write = cv2.imread('./datasets/ic13/test/img/'+ im_w_name)[:, :,::-1]
                    ...
                    if 'ic15' in cfg.DATASETS.TEST[0]:
                        cv2.imwrite('./visualize/ic15/' + im_w_name, im_write[:, :, ::-1])
                    elif 'ctst' in cfg.DATASETS.TEST[0]:
                        cv2.imwrite('./visualize/ctst/' + im_w_name, im_write[:, :, ::-1])
                    elif 'tpd' in cfg.DATASETS.TEST[0]:
                        cv2.imwrite('./visualize/tpd/' + im_w_name, im_write[:, :, ::-1])
                    elif 'ic13' in cfg.DATASETS.TEST[0]:
                        cv2.imwrite('./visualize/ic13/' + im_w_name, im_write[:, :, ::-1])
                    else:
                        cv2.imwrite('./visualize/odett/' + im_w_name, im_write[:, :, ::-1])
